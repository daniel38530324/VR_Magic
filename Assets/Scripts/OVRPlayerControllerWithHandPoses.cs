...
public class OVRPlayerControllerWithHandPoses : MonoBehaviour
{
	...
	public virtual void UpdateMovement()
	{
		...
		if (EnableLinearMovement)
		{
			...
			if (OVRInput.Get(OVRInput.Button.DpadLeft) || HandPoseRotateLeft)
			{
				moveLeft = true;
				dpad_move = true;
			}

			if (OVRInput.Get(OVRInput.Button.DpadRight) || HandPoseRotateRight)
			{
				moveRight = true;
				dpad_move = true;
			}
			...
			if (moveLeft)
				MoveThrottle += ort * (transform.lossyScale.x * moveInfluence * 0.7f * Vector3.left);
			if (moveRight)
				MoveThrottle += ort * (transform.lossyScale.x * moveInfluence * 0.7f * Vector3.right);
